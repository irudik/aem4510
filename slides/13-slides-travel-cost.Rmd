...
<!-- Auto-rebuild: ls slides/13-slides-travel-cost.Rmd | entr -c Rscript -e "rmarkdown::render('slides/13-slides-travel-cost.Rmd')" -->
...
.clean-table th {
  border-bottom: 2px solid #3C93DC;
  color: #1f2d3d;
  font-weight: 700;
  padding: 6px 8px;
  text-align: left;
...
```
```{r table_helpers}
clean_table = function(data, col_names = names(data), digits = 3) {
  knitr::kable(
    data,
    format = "html",
    col.names = col_names,
...
```{r, echo = FALSE}
local_video("videos/13-travel-cost-great-lakes-carp.mp4")
```
...
- Hourly wage: w
- Money cost of reaching the site: c; travel time to the site: t
...
<div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 10px; margin-top: 18px; font-size: 0.62em;">
  <div style="border-left: 5px solid #3C93DC; padding: 8px 10px; background: #f7f8fa;">
    <div style="font-weight: 700; font-size: 1.2em; margin-bottom: 6px;">Zone A</div>
    <div>Distance: 2</div>
    <div>Population: 10,000</div>
    <div>Travel cost: 20</div>
    <div>Visits per person: 15</div>
  </div>
  <div style="border-left: 5px solid #3C93DC; padding: 8px 10px; background: #f7f8fa;">
    <div style="font-weight: 700; font-size: 1.2em; margin-bottom: 6px;">Zone B</div>
    <div>Distance: 30</div>
    <div>Population: 10,000</div>
    <div>Travel cost: 30</div>
    <div>Visits per person: 13</div>
  </div>
  <div style="border-left: 5px solid #3C93DC; padding: 8px 10px; background: #f7f8fa;">
    <div style="font-weight: 700; font-size: 1.2em; margin-bottom: 6px;">Zone C</div>
    <div>Distance: 90</div>
    <div>Population: 20,000</div>
    <div>Travel cost: 65</div>
    <div>Visits per person: 6</div>
  </div>
  <div style="border-left: 5px solid #3C93DC; padding: 8px 10px; background: #f7f8fa;">
    <div style="font-weight: 700; font-size: 1.2em; margin-bottom: 6px;">Zone D</div>
    <div>Distance: 140</div>
    <div>Population: 10,000</div>
    <div>Travel cost: 80</div>
    <div>Visits per person: 3</div>
  </div>
  <div style="border-left: 5px solid #3C93DC; padding: 8px 10px; background: #f7f8fa;">
    <div style="font-weight: 700; font-size: 1.2em; margin-bottom: 6px;">Zone E</div>
    <div>Distance: 150</div>
    <div>Population: 10,000</div>
    <div>Travel cost: 90</div>
    <div>Visits per person: 1</div>
  </div>
</div>
...
The demand curve shifts with $(\alpha_{00}, \alpha_{01})$ and rotates with $(\alpha_{10}, \alpha_{11})$ as we change site attribute $z$
...
trip_data |> 
  ...
  transmute(
    ...
    Trips = trips,
    Income = round(income),
    `Travel cost` = round(travel_cost, 1),
    `Other-site cost` = round(travel_cost_other, 1),
    `Water clarity` = round(water_clarity, 2)
  ) |> 
  clean_table()
```
...
estimation_df <- site_estimates %>% 
  left_join(trip_data %>% distinct(site, water_clarity), by = "site")
estimation_df
...
demand_shifts |> 
  ...
  transmute(
    Term = "Water clarity",
    Estimate = if_else(coeff == "Income", estimate * 10000, estimate),
    `Demand parameter` = if_else(coeff == "Income", "Income (per $10,000)", coeff)
  ) |> 
  clean_table()
```
The estimates column tells us how a change in water clarity (from 0 to 100%), shifts or rotates our demand curve

The income effect is scaled to a $10,000 change in income.
