...
There is no clear way to value changes in quantities of environmental goods
...
# The hedonic model: the price function
...
ggplot() +
  ...
  labs(x = "Quantity of Environmental good (q)",
       y = "Price of Property ($)",
       title = "The Hedonic Price Function") +
...
The hedonic price function is $P(x,q)$
...
ggplot() +
  ...
  labs(x = "Quantity of Environmental good (q)",
       ...
       title = "The Implicit Price Function") +
...
The implicit price function for q is ${\partial P(x,q) \over \partial q}$
...
The hedonic price function is the upper envelope of all the bid functions
...
# Hedonic price function and willingness to pay

The hedonic price function shows how a house varies with attributes (such as air quality)
...
At a given point on the function, the slope also gives the marginal WTP of the consumer who purchase that level of attributes
...
Their bid curve is only tangent at that one point, we don't see their whole bid curve
...
They regress:
$$log(2000 median home price)_c = \theta 1(cleaned up in 2000)_c + \beta X_c + \varepsilon_{c}$$
...
# Superfund results: conventional approach

<center>
```{r, out.width = "50%", fig.pos="c", echo = FALSE}
knitr::include_graphics("files/11-superfund-results1.png")
```
</center>
...
Superfund cleanups had economically and statistically insignificant effects on property values, rental rates, housing supply, population, who lives near the site
...
Previous attempts to value cleanup looked at changes incensus tract median housing values and found no impacts
...
For instance, if in the new equilibrium houses on Lake A sell for \\$210,000, while house on Lake B sell for \\$200,000, the “implicit price” associated with the increased water clarity is \\$10,000.
...
Just like our hedonic price function, we can estimate a hedonic wage function:
...
This means that:
$$VSL = (0.4/10,000) * 1000) = 4 \text{ million dollars}$$
