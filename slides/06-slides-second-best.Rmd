...
They have opposing forces on quantities, so the market failures offset each other to some degree
...
- Cobb-Douglas production using labor and emissions as inputs: $L^\alpha E^{1-\alpha}$
...
$$\max_{L,E} (p-\tau)L^\alpha E^{1-\alpha} - wL - rE - dE$$

--

The first-order conditions for a socially efficient allocation of $L$ and $E$ are:
$$\alpha p L^{\alpha-1} E^{1-\alpha} = w \qquad (1-\alpha) p L^{\alpha} E^{-\alpha} = r + d$$
...
$$\max_{L,E} (p-\tau)L^\alpha E^{1-\alpha} - wL - rE$$
...
The output tax penalizes the use of clean labor despite it not causing any externalities: $\tau_o L^{\alpha-1} E^{1-\alpha}$, this is .hi-red[not efficient]
...
<center>
```{r, out.width = "100%", fig.pos="c", echo = FALSE}
knitr::include_graphics("files/06-flaring1.png")
```
</center>
...
<center>
```{r, out.width = "100%", fig.pos="c", echo = FALSE}
knitr::include_graphics("files/06-flaring2.png")
```
</center>
...
knitr::include_graphics("files/06-flaring3.png")
```
</center>

---

# Intensity standards: Flaring
...
For the standard to make any sense it must be: $\beta_L \leq \sigma \leq \beta_H$, otherwise it doesn't do anything $\sigma > \beta_H$ or its unattainable $\sigma < \beta_L$
...
<center>
```{r, out.width = "100%", fig.pos="c", echo = FALSE}
knitr::include_graphics("files/06-flaring4.png")
```
</center>
]

.pull-right[
Moving from a flaring intensity standard to tradable permits or a tax generates massive welfare gains!

A tax that captures the same quantity of gas would have >40% lower cost than the actual intensity standard
