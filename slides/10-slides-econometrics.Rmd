...
<!-- Auto-rebuild: ls slides/10-slides-econometrics.Rmd | entr -c Rscript -e "rmarkdown::render('slides/10-slides-econometrics.Rmd')" -->
...
```{R, ols vs lines 1, echo = F, dev = "svg", fig.height = 4.5, fig.width = 10, fig.align = 'center'}
...
```{R, vs lines 2, echo = F, dev = "svg", fig.height = 4.5, fig.width = 10, fig.align = 'center'}
# Define a function
...
# The plot
ggplot(data = pop_df, aes(x = x, y = y)) +
  # geom_segment(aes(x = x, xend = x, y = y, yend = y_hat(x, b0, b1)), linewidth = 0.5, alpha = 0.2) +
  geom_point(size = 5, color = "darkslategray", alpha = 0.9) +
  geom_abline(intercept = b0, slope = b1, color = "#3C93DC", linewidth = 2, alpha = 0.9) +
  theme_empty +
  labs(y = "GPA", x = "Average Annual Lead Exposure (grams)") +
  coord_cartesian(ylim = c(-5, 5))
```
...
```{R, ols vs lines 3, echo = F, dev = "svg", fig.height = 4.5, fig.width = 10, fig.align = 'center'}
# Define a function
y_hat = function(x, b0, b1) {b0 + b1 * x}
# Define line's parameters
b0 = 1
...
```{R, ols vs lines 4, echo = F, dev = "svg", fig.height = 4.5, fig.width = 10, fig.align = 'center'}
# Define a function
y_hat = function(x, b0, b1) {b0 + b1 * x}
# Define line's parameters
b0 = 3
...
```{R, ols vs lines 5, echo = F, dev = "svg", fig.height = 4.5, fig.width = 10, fig.align = 'center'}
# Define a function
y_hat = function(x, b0, b1) {b0 + b1 * x}
# Define line's parameters
b0 = 2
...
```{R, ols vs lines 6, echo = F, dev = "svg", fig.height = 4.5, fig.width = 10, fig.align = 'center'}
# Define a function
y_hat = function(x, b0, b1) {b0 + b1 * x}
# Define line's parameters
b0 = 2
...
# The plot
ggplot(data = pop_df, aes(x = x, y = y)) +
  geom_segment(aes(x = x, xend = x, y = y, yend = y_hat(x, b0, b1), color = (y - y_hat(x, b0, b1))^2), linewidth = 0.5, alpha = 0.8) +
  geom_point(size = 5, color = "darkslategray", alpha = 0.9) +
  geom_abline(intercept = b0, slope = b1, color = "#3C93DC", linewidth = 2, alpha = 0.9) +
...
```{R, ols vs lines 7, echo = F, dev = "svg", fig.height = 4.5, fig.width = 10, fig.align = 'center'}
# Define a function
y_hat = function(x, b0, b1) {b0 + b1 * x}
# Define line's parameters
...
ggplot(data = pop_df, aes(x = x, y = y)) +
  geom_segment(aes(x = x, xend = x, y = y, yend = y_hat(x, b0, b1), color = (y - y_hat(x, b0, b1))^2), linewidth = 0.5, alpha = 0.8) +
  geom_point(size = 5, color = "darkslategray", alpha = 0.9) +
  geom_abline(intercept = b0, slope = b1, color = "#ff0000", linewidth = 2, alpha = 0.9) +
  scale_color_viridis(option = "cividis", direction = -1) +
  theme_empty +
  labs(y = "GPA", x = "Average Annual Lead Exposure (grams)") +
  coord_cartesian(ylim = c(-5, 5))
...
$$ \text{GPA}_i = `r round(lm0$coefficients[1], 2)` + `r round(lm0$coefficients[2], 2)` P_i + u_i $$

$$ \text{GPA}_i = \beta_0 + \beta_1 P_i + u_i $$
...
**Sample relationship**
<br>
$\widehat{\text{GPA}}_i = `r round(lm1$coefficients[1], 2)` + `r round(lm1$coefficients[2], 2)` P_i$
...
geom_abline(
intercept = lm1$coefficients[1], slope = lm1$coefficients[2],
linewidth = 2, linetype = 2, color = "black", alpha = 0.3
) +
geom_abline(
...
**Sample relationship**
<br>
$\widehat{\text{GPA}}_i = `r round(lm2$coefficients[1], 2)` + `r round(lm2$coefficients[2], 2)` P_i$
...
**Sample relationship**
<br>
$\widehat{\text{GPA}}_i = `r round(lm3$coefficients[1], 2)` + `r round(lm3$coefficients[2], 2)` P_i$
...
.pull-left[ 
  ...
  ggplot() + 
    geom_abline(data = line_df, aes(intercept = intercept, slope = slope), alpha = 0.01, linewidth = 1) +
...
Poorer households are exposed to more lead **and** have fewer tutoring resources

--

$\hat{\beta}_1$ will pick up the effect of both!
...
ggplot(tri) +
  ...
  geom_vline(xintercept = median(tri$emissions), color = "black", linetype = "dotdash", linewidth = 2) +
  geom_vline(xintercept = 1.53, color = "#ff0000", linewidth = 2) +
  geom_vline(xintercept = 1.75, color = "#FFAC0F", linetype = "longdash", linewidth = 2) +
  geom_vline(xintercept = 6.04, color = "#FFAC0F", linetype = "longdash", linewidth = 2) +
...
nascar_df |>
  select(school_id, school_name, grade, year, zscore, nascar_lead, industrial_lead, median_income, num_students) |>
  slice_head(n = 10) |>
  mutate(
    zscore = round(zscore, 3),
    nascar_lead = round(nascar_lead, 2),
    industrial_lead = round(industrial_lead, 2),
    median_income = round(median_income, 1)
  ) |>
  knitr::kable(
    format = "html",
    col.names = c("School ID", "School", "Grade", "Year", "Test avg", "NASCAR lead", "Industrial lead", "Median income", "Students")
  ) |>
  kableExtra::kable_styling(font_size = 18, full_width = FALSE, position = "center")
```
...
- .hi[test avg]: the school's score for the average student in terms of standard deviations above or below the state-wide average
...
bin_df = nascar_df |>
  mutate(
    lead_bin = cut_interval(nascar_lead_weighted, n = 50, labels = FALSE)
  ) |>
  group_by(lead_bin) |>
  summarise(
    mean_lead = mean(nascar_lead_weighted),
    mean_score = mean(zscore)
  ) |>
  ungroup()

ggplot(bin_df, aes(x = mean_lead, y = mean_score)) +
...
It appears there's a .hi-red[negative] association: lead is bad for test scores
...
$$test\ avg_{sgy} = \beta_0 + \beta_1 nascar\_lead\_weighted_{sgy}$$ $(s$ is school, $g$ is grade, $y$ is year)
...
lm(zscore ~ nascar_lead_weighted, nascar_df) |>
  broom::tidy() |>
  transmute(
    Parameter = c("beta_0 (Intercept)", "beta_1 NASCAR lead"),
    Estimate = round(estimate, 4),
    `Std. error` = round(std.error, 4),
    `t value` = round(statistic, 2)
  ) |>
  knitr::kable(format = "html", caption = "Estimation results") |>
  kableExtra::kable_styling(font_size = 18, full_width = FALSE, position = "center")
```
...
$$test\ avg_{sgy} = \beta_0 + \beta_1 nascar\_lead\_weighted_{sgy} + \beta_2 other\_lead_{sgy}  + \beta_3 income_{sgy}$$


```{r, echo = FALSE, results = 'asis'}
reg_table = tibble::tibble(
  Parameter = c("beta_0 (Intercept)", "beta_1 NASCAR lead", "beta_2 Other lead", "beta_3 Income"),
  Estimate = c("-0.846", "-0.0008", "-0.00000006", "0.00002"),
  Note = c("", "(versus -0.004 above)", "(other lead = bad!)", "(rich family = good!)")
)

reg_table |>
  knitr::kable(format = "html", caption = "Estimation results with controls") |>
  kableExtra::kable_styling(font_size = 18, full_width = FALSE, position = "center")
```
...
ggplot(bin_df, aes(x = mean_lead, y = mean_income)) +
  geom_point(size = 5, color = "darkslategray") +
  geom_smooth(formula = y ~ x, color = red_pink, method = "lm", se = FALSE) +
  theme_regular +
  scale_y_continuous(breaks = scales::pretty_breaks()) +
