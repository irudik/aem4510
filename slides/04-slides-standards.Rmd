...
The MB of emissions is given by the MAC (avoided abatement cost, an opportunity cost)

The MC of emissions without regulation is....zero

So firms set emissions where: MB=MC $\rightarrow$ MAC = 0
...
To figure out the socially optimal standard we need to know how the firm and regulator make decisions
...
If $C(E)$ is decreasing in $E$, but $E$ is bounded above by $\bar{E}$....
...
$$\min_{\bar{E}} C(E) + D(E) \,\,\, \text{subject to: } \,\,\, E = \bar{E}$$
...
The regulator chooses $\bar{E}$ to be the emission level where MAC = MD, $E^*$

--

$-C'(\bar{E})$ is the MAC since $-C'(E) = C'(A)$ where $E = -A$
...
The previous example assumed firms couldn't lie/cheat and **not** abate
