...
But in Ithaca in January, *there's some randomness in how cold it is on any given day relative to long-run average January temperature*

--

This long-run average temperature/weather is what we call the .hi[climate]: $C_{it}$
...
When farmers decide to plant in spring, they can't predict deviations from average weather during the summer growing season for any day

--

The deviations from average are effectively random from the farmer's perspective

---

# Weather vs climate

If weather is random, then we can estimate the .hi-blue[marginal effect of weather]
...
One reasonable guess is that the effect of weather provides an upper bound on the effect of climate change
...
Climate change is a long-run phenomenon: in the long-run we might have more ways to adapt
...
.hi[Note:] we did see an example earlier for why the bias might go the other way
...
Because farmland values shouldn't change in response to random annual weather shocks
  - Since they're random and transient, not permanent changes, they won't have lasting effects on land values
