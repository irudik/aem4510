...
source("R/08-second-best-intensity-figures.R")
...
knitr::opts_chunk$set(
  cache = TRUE,
  cache.extra = list(tools::md5sum("R/08-second-best-intensity-figures.R")),
  echo = TRUE,
  fig.align = "center"
)
  options
...
Here:
- $\beta_H$ and $\beta_L$ are emissions coefficients: each unit of $H$ emits $\beta_H$ units and each unit of $L$ emits $\beta_L$ units
- $\sigma$ is the policy target: the maximum average emissions rate per unit of total output
- Usually $\beta_H > \sigma > \beta_L$, so $H$ is dirtier than the standard while $L$ is cleaner than the standard
...
# Intensity standards: firm's problem

Suppose the firm takes output prices $p_H$ and $p_L$ as given

--

Its private problem is:
$$\max_{Q_H,Q_L} \pi(Q_H,Q_L) = p_H Q_H + p_L Q_L - C_H(Q_H) - C_L(Q_L)$$

--

subject to the intensity standard:
$$\frac{\beta_H Q_H + \beta_L Q_L}{Q_H + Q_L} \leq \sigma$$

---

# Intensity standards: Lagrangian setup

Rewrite the constraint as:
$$\sigma(Q_H + Q_L) - \beta_H Q_H - \beta_L Q_L \geq 0$$

--

The Lagrangian is:
$$\mathcal{L} = p_H Q_H + p_L Q_L - C_H(Q_H) - C_L(Q_L) + \lambda\left[\sigma(Q_H + Q_L) - \beta_H Q_H - \beta_L Q_L\right]$$

--

with $\lambda \geq 0$; if the standard binds, $\lambda$ is the shadow value of relaxing it

---

# Intensity standards: firm incentives

Taking first-order conditions from the Lagrangian gives:
...
# Intensity standards: baseline DWL

.pull-left[
<div style="margin-top:-28px;">
```{r intensity-markets-baseline-dwl, out.width = "80%", fig.width = 5.8, fig.height = 8.4, dpi = 180, echo = FALSE, fig.align = "center"}
sb_plot_intensity_market_pair(show_policy = FALSE, dwl_mode = "baseline", ncol = 1)
```
</div>
]

.pull-right[
Start from the unregulated allocation $(q_H^0, q_L^0)$:
- Each market overproduces relative to $q_X^*$ because firms ignore $\beta_X MD$
- The red triangle is baseline DWL from moving from $q_X^*$ to $q_X^0$
- At this stage there is .hi[no] implicit tax or subsidy layered onto the market graph yet
]

---

# Intensity standards: separate output markets

.pull-left[
<div style="margin-top:-28px;">
```{r intensity-markets, out.width = "80%", fig.width = 5.8, fig.height = 8.4, dpi = 180, echo = FALSE, fig.align = "center"}
sb_plot_intensity_market_pair(show_policy = TRUE, dwl_mode = "none", ncol = 1)
```
</div>
]

.pull-right[
Market-by-market intuition:
- In market $X \in \{H,L\}$, PMB is the flat price line $p_X$ and PMC is the upward-sloping cost curve $MC_X$
- Social marginal cost is $SMC_X = MC_X + \beta_X MD$
- The intensity standard shifts PMC .hi-red[up] for $H$ and .hi-blue[down] for $L$
]

---

# Intensity standards: DWL changes by market

.pull-left[
<div style="margin-top:-28px;">
```{r intensity-markets-dwl, out.width = "80%", fig.width = 5.8, fig.height = 8.4, dpi = 180, echo = FALSE, fig.align = "center"}
sb_plot_intensity_market_pair(show_policy = TRUE, dwl_mode = "baseline", ncol = 1)
```
</div>
]
...
- The red triangle is baseline DWL under no policy
]
...
.pull-left[
<div style="margin-top:-28px;">
```{r intensity-markets-dwl-2, out.width = "80%", fig.width = 5.8, fig.height = 8.4, dpi = 180, echo = FALSE, fig.align = "center"}
sb_plot_intensity_market_pair(show_policy = TRUE, dwl_mode = "change", ncol = 1)
```
</div>
]
...
Relative to no policy:
- The red triangle is baseline DWL under no policy
- The blue triangle shows the DWL after the implicit tax/subsidy wedges are layered onto the markets
- Both goods still pollute, so the subsidy on $L$ keeps the policy from reaching the socially optimal allocation
]
...
.pull-left[
<div style="margin-top:-28px;">
```{r intensity-markets-dwl-3, out.width = "80%", fig.width = 5.8, fig.height = 8.4, dpi = 180, echo = FALSE, fig.align = "center"}
sb_plot_intensity_market_pair(show_policy = TRUE, dwl_mode = "change", ncol = 1)
```
</div>
]
...
.pull-right[
Relative to no policy:
- The implicit tax on $H$ shifts $MC_H$ up and moves output toward $q_H^*$, so DWL falls in the $H$ market
- The implicit subsidy on $L$ shifts $MC_L$ down and moves output away from $q_L^*$, so DWL rises in the $L$ market
]
...
So it can improve on no policy, but usually cannot hit the socially optimal allocation
