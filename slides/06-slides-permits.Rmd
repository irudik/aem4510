...
# Each firm's permit demand sets its own MAC equal to the permit price.
mac1 = function(x) 90 - x
mac2 = function(x) 150 - 2*x
high_price = 70
low_price = 50
...
ggplot() +
  ...
  stat_function(fun = mac2, xlim = c(0, 75), color = "#638ccc", linewidth = 1.5) +
  annotate("text", x = 12, y = 92, label = "MAC[1]", parse = TRUE,
           color = "#ca5670", size = 8, hjust = 0) +
  annotate("text", x = 12, y = 142, label = "MAC[2]", parse = TRUE,
           color = "#638ccc", size = 8, hjust = 0) +
  annotate("segment", x = 0, xend = (150 - high_price)/2, y = high_price, yend = high_price,
  ...
  annotate("segment", x = 0, xend = (150 - low_price)/2, y = low_price, yend = low_price,
           linetype = "dotted", linewidth = 1.3) +
  annotate("segment", x = 90 - low_price, xend = 90 - low_price, y = 0, yend = low_price,
           linetype = "dashed", linewidth = 1.3, color = "grey50") +
  annotate("segment", x = (150 - high_price)/2, xend = (150 - high_price)/2,
           y = 0, yend = high_price, linetype = "dashed", linewidth = 1, color = "#638ccc") +
  annotate("segment", x = (150 - low_price)/2, xend = (150 - low_price)/2,
           y = 0, yend = low_price, linetype = "dashed", linewidth = 1, color = "#638ccc") +
  annotate("point", x = c(90 - high_price, 90 - low_price), y = c(high_price, low_price),
           color = "#ca5670", size = 4) +
  annotate("point", x = c((150 - high_price)/2, (150 - low_price)/2), y = c(high_price, low_price),
           color = "#638ccc", size = 4) +
  permit_auction_theme +
  labs(x = "Permits held = emissions", y = "$ per permit") +
  scale_x_continuous(expand = c(0, 0), limits = c(0, 95), breaks = c(20, 40, 50, 75, 90)) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 160), breaks = c(low_price, high_price, 90, 150))
```
...
At $p = 70$: wants 20 permits<br>At $p = 50$: wants 40 permits

Firm 2: $MAC_2 = 150 - 2E_2$

Setting $MAC_2 = p$ gives $E_2 = 75 - p/2$<br>At $p = 70$: wants 40 permits<br>At $p = 50$: wants 50 permits
]
...
<img src="`r knitr::fig_chunk("permits-auction-supply-demand", "png")`" alt="permit auction supply and demand" style="width:90%;">
]

.pull-right[
.hi-blue[Supply]: the cap, $\bar{E} = 90$

.hi-blue[Firm 2]: $MAC_2 = 150 - 2E_2$<br>At price $p$, it wants $75 - p/2$ permits
...
Three firms bid for .hi[5 permits]:
...
# Rank the three firms' submitted bids for the 5 permits for sale.
...
All winners pay only .hi[7 per permit], even when they bid higher
...
# Permit auctions: the gain to the firm
...
.pull-right[
A permit avoids one unit of abatement. The firm's .hi[gain] is the avoided MAC minus the price paid:

$$\text{Gain} = MAC - p$$

Example: $12 - 7 =$ .hi[5 gained per permit]

The bars show bids, not necessarily MACs
]
...
# The permit market game: auctions and trading
...
No banking or borrowing: each round's permits cover only that round's emissions
...
- A higher Round 2 price when the cap tightens, holding MACs fixed
...
# The permit market game: scoring

Your .hi[round score] is the abatement cost your permits save, plus permit sales revenue, minus spending on permits

$$\text{Score} = \text{avoided abatement cost} + \text{sales} - \text{purchases}$$

Avoided cost is relative to abating all baseline emissions. Purchases include auction payments, which are zero with free allocation

.hi[Leaderboard points] = your score minus your firm's benchmark score, summed over rounds

The benchmark gives your firm its cost-effective permit quantity at the cost-effective price, accounting for its free permits

---

# Join the permit market game

.center[
Scan to open the student login, then join with your team name

<img src="files/06-permit-game-qr.png" alt="QR code for the permit market student login" width="30%" />

Or open .hi[[aem4510.ivanrudik.com/permits](https://aem4510.ivanrudik.com/permits)]
]
...
class: inverse, center, middle
name: permit-game-banking

# The permit game: banking and borrowing

<html><div style='float:left'></div><hr color='#EB811B' size=1px width=796px></html>

---

# The permit game: decisions across rounds

We now allow permits to move between the two rounds

--

.hi[Bank:] emit less than your permits cover in Round 1 and save the unused permits for Round 2

--

.hi[Borrow:] emit more than your permits cover in Round 1 and repay the difference with Round 2 permits

--

Use the emissions choice in Round 1. Unrepaid borrowing pays a penalty at the end of Round 2

---

# The permit game: predictions across rounds

How should expected Round 2 conditions change your Round 1 decisions?

--

- Bank when permits are more valuable in Round 2
- Borrow when permits are more valuable in Round 1, accounting for repayment
- Trading across rounds brings marginal abatement costs closer together

--

Compare these results with the game without banking and borrowing

---

# Rejoin the permit game: banking and borrowing
...
# Debrief: banking and borrowing

Did teams bank before the tighter Round 2 cap? Did any teams borrow instead?

--

Compare your MAC across rounds: could shifting one more permit have reduced your total abatement cost?

--

Banking and borrowing extend the gains from trade across time
