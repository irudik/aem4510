...
<!-- Auto-rebuild: ls slides/12-slides-hedonics.Rmd | entr -c Rscript -e "rmarkdown::render('slides/12-slides-hedonics.Rmd')" -->
...
```{r, echo = FALSE}
local_video("videos/12-hedonics-superfund.mp4")
```
...
knitr::include_graphics("files/14-woburn.png")
```
</center>
...
Analogous to MRS = price ratio of two goods from intermediate micro, but here the price of z is just $1
...
.hi[This means that the MRS between z and q is the WTP for] $q$: the reduction in income needed to compensate for an additional unit of $q$, while maintaining the same level of utility
...
We are .hi[differencing out] other things that may affect housing prices
...
.hi[How to answer it:] Compare housing market outcomes in the areas surrounding the first 400 hazardous sites chosen for Superfund clean-ups to the areas surrounding the 290 sites that narrowly missed qualifying for these clean-ups
...
Because of funding limits, we have a cutoff: HRS > 28.5 cleaned up, HRS < 28.5 are not
...
knitr::include_graphics("files/14-superfund-location.png")
```
</center>
...
They regress:
$$\log(\text{2000 median home price})_c = \theta \underbrace{1(\text{cleaned up in 2000})_c}_{\text{= 1 if cleaned up, = 0 if not}} + \beta \underbrace{X_c}_{\text{controls}} + \varepsilon_{c}$$
...
<center>
```{r, out.width = "80%", fig.pos="c", echo = FALSE}
knitr::include_graphics("files/14-superfund-results2.png")
```
</center>
...
Suggests the mean benefits of Superfund cleanup are small: at most, people demand only 3% lower housing prices to live near toxic waste!
...
<center>
```{r, out.width = "50%", fig.pos="c", echo = FALSE}
knitr::include_graphics("files/14-vsl-estimates.png")
```
</center>
